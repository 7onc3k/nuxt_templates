<template>
    <div class="relative">
        <div :class="{'h-screen': isOpen, 'h-20': !isOpen, 'lg:h-24': !isOpen}" class="transition-all duration-300 fixed top-0 left-0 w-full bg-blue-600 flex flex-col justify-between">
            ...
            <div v-if="!selectedLink" class="flex justify-between items-center px-4 lg:px-8 absolute top-0 left-0 w-full h-20 lg:h-24">
            ...
            <div v-if="isOpen" class="flex flex-col justify-center items-center flex-grow space-y-8 lg:space-y-12 mt-24 lg:mt-28">
                <a href="#works" @click="navigate('works', $event)" :class="{ 'selected': selectedLink === 'works', 'move': move && selectedLink === 'works', 'fade-out': fadeOut }" class="text-white text-4xl lg:text-5xl">Works</a>
                <a href="#pricing" @click="navigate('pricing', $event)" :class="{ 'selected': selectedLink === 'pricing', 'move': move && selectedLink === 'pricing', 'fade-out': fadeOut }" class="text-white text-4xl lg:text-5xl">Pricing</a>
                <a href="#contact" @click="navigate('contact', $event)" :class="{ 'selected': selectedLink === 'contact', 'move': move && selectedLink === 'contact', 'fade-out': fadeOut }" class="text-white text-4xl lg:text-5xl">Contact</a>
            </div>
            
            <div v-if="isOpen && !selectedLink" @click="toggleMenu" class="cursor-pointer flex justify-end items-center h-20 lg:h-24 px-4 lg:px-8 mr-4 lg:mr-0">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 lg:w-8 lg:h-8 text-white" height="1em"
                    viewBox="0 0 448 512">
                    <path
